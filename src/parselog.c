...
/**
  * function:返回日志来源，包含“&_x=”的日志判定为来源于定位，
  *          加上‘&’是因为cormorant日志里有些日志
  *          包含“lu=”的即判定为cormorant行为日志来的信息
  * input   :一个字符串指针
  * retrun  :返回值为0，1，2，0为无效数据，1为tk的日志，2为cor的日志
  */
...
/* 
 * function: 处理每条日志的参数，调用一次处理一个参数，放到log结构中
 * input   : log结构指针，key，value指针，放入到log的list参数值
 *           的key会被free掉，无需再处理
 * return  ：无返回值，操作传入的log
 * */
